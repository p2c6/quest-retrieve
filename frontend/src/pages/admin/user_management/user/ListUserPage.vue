...
const openDeleteUserConfirmation = (id) => {
...
const search = async() => {
    userStore.keyword = formData.keyword;
        ...
        <div class="overflow-x-auto w-full">
            <div class="flex flex-col gap-2 justify-between items-center md:flex-row">
                <div class="text-center md:text-left">
                    <p class="text-primary font-medium">User List</p>
                    <p class="text-tertiary text-xs md:text-sm">Listing of all users.</p>
                </div>
            ...
            <div class="w-full mt-5 relative overflow-x-auto shadow-md sm:rounded-lg">
                <div v-if="userStore.message" class="bg-teal-100 border-t-4 border-teal-500 rounded-b text-teal-900 px-4 py-3 shadow-md mb-5" role="alert">
                        <div class="flex">
                            <div class="py-1">
                    ...
                    <div v-if="userStore.errors && userStore.errors.message" class="bg-red-100 border-t-4 border-red-500 rounded-b text-red-900 px-4 py-3 shadow-md mb-5" role="alert">
                        <div class="flex">
                            <div class="py-1">
                            ...
                            <div>
                                ...
                                <div class="text-xs flex gap-1">{{ userStore.errors.message }}
                                </div>
                            </div>
                        </div>
                    </div>
                    <div role="status" v-if="userStore.isLoading" class="flex items-center justify-center mt-10">
                            ...
                            <tr v-if="userStore.users.data && userStore.users.data.length > 0" v-for="user in userStore.users.data" :key="user.id" class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex gap-2">
                                        <RouterLink :to="{name: 'users.edit', params:{ id: user.id } }" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                            <i class="text-primary pi pi-pen-to-square cursor-pointer"> Edit</i>
                                        </RouterLink>
                                        <div class="text-red-500 cursor-pointer" @click="openDeleteUserConfirmation(user.id)">
                    ...
                    <div class="flex justify-center md:justify-end mt-2">
                        ...
                        @pagination-change-page="userStore.getAllUsers"
                    />
                    </div>
            </div>
        </div>
