...
const openDeleteSubcategoryConfirmation = (id) => {
...
const search = async() => {
    subcategoryStore.keyword = formData.keyword;
                ...
                <div id="modal-body" class="flex items-center justify-center px-4">
                    <div class="flex-1">
                        <div class="flex flex-col justify-center gap-2">
                            <p class="font-semibold text-md text-primary">Delete subcategory?</p>
                            <p class="text-xs text-gray-400">You are about to delete this subcategory</p>
                            <div class="flex gap-1 mt-2">
                                <button class="w-36 h-8 bg-gray-200 rounded text-sm" @click="closeModal">Cancel</button>
                                <button class="w-36 h-8 bg-secondary rounded text-white text-sm" @click="confirmDeleteSubcategory(subcategoryId)">Yes,delete it</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="overflow-x-auto w-full">
            <div class="flex flex-col gap-2 justify-between items-center md:flex-row">
                <div class="text-center md:text-left">
                    <p class="text-primary font-medium">Subcategory List</p>
                    <p class="text-tertiary text-xs md:text-sm">Listing of all subcategories.</p>
                </div>
                <div class="w-full md:w-16">
                    <RouterLink :to="{ name: 'subcategory.create' }">
            ...
            <div class="w-full mt-5 relative overflow-x-auto shadow-md sm:rounded-lg">
                <div v-if="subcategoryStore.message" class="bg-teal-100 border-t-4 border-teal-500 rounded-b text-teal-900 px-4 py-3 shadow-md mb-5" role="alert">
                        <div class="flex">
                            <div class="py-1">
                            ...
                            <div class="text-xs flex gap-1">
                                {{ subcategoryStore.message }}
                            </div>
                            </div>
                        </div>
                    </div>
                    <div v-if="subcategoryStore.errors && subcategoryStore.errors.message" class="bg-red-100 border-t-4 border-red-500 rounded-b text-red-900 px-4 py-3 shadow-md mb-5" role="alert">
                        <div class="flex">
                            <div class="py-1">
                            ...
                            <div>
                                ...
                                <div class="text-xs flex gap-1">{{ subcategoryStore.errors.message }}
                                </div>
                            </div>
                        </div>
                    </div>
                    <div role="status" v-if="subcategoryStore.isLoading" class="flex items-center justify-center mt-10">
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400" v-else>
                        ...
                        <tbody>
                            <tr v-if="subcategoryStore.subcategories.data.length > 0" v-for="subcategory in subcategoryStore.subcategories.data" :key="subcategory.id" class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex gap-2">
                                        <RouterLink :to="{name: 'subcategory.edit', params:{ id: subcategory.id } }" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                            <i class="text-primary pi pi-pen-to-square cursor-pointer"> Edit</i>
                                        </RouterLink>
                                        <div class="text-red-500 cursor-pointer" @click="openDeleteSubcategoryConfirmation(subcategory.id)">
                    ...
                    <div class="flex justify-center md:justify-end mt-2">
                        ...
                        @pagination-change-page="subcategoryStore.getAllSubcategories"
                    />
                    </div>
            </div>
        </div>
