...
class PostTest extends TestCase
{
     ...
     */
    public function test_user_cannot_store_post_with_all_empty_fields(): void
    {
        $role = Role::where('id', UserType::PUBLIC_USER)->first();
    ...
    /**
     * Test app can send post created mail upon post creation via API.
     * 
     * This test verifies that the app can send post created mail upon post creation via API endpoint.
     */
    public function test_app_can_send_post_created_mail_upon_post_creation(): void
    {
        $role = Role::where('id', UserType::PUBLIC_USER)->first();
    ...
    /**
     * Test user cannot update post while the status is not pending via API.
     * 
     * This test verifies that a user cannot update post while the status is not pending via API endpoint.
     */
    public function test_user_cannot_update_post_while_status_not_pending(): void
    {
        $role = Role::where('id', UserType::PUBLIC_USER)->first();
    ...
    public function test_user_cannot_mark_post_as_done_if_not_owner(): void
    {
        ...
        $response = $this->putJson(route('api.v1.posts.markAsDone', $otherPost->id), [
            'user_id' => $user->id,
            'type' => $updatedType,
            'subcategory_id' => $updatedSubCategoryId,
            'incident_location' => $updatedIncidentLocation,
